...
struct VerbindSheet: View {
    ...
    private var ringVoortgang: Double {
        max(min(bluetooth.importVoortgang, 1), 0.02)
    }

    private var ontvangenTekst: String {
        let aantal = bluetooth.ontvangenMetingAantal
        guard aantal > 0 else {
            return bluetooth.importModus == .laatsteMeting
                ? "Wachten op nieuwe meting"
                : "Nog geen metingen ontvangen"
        }

        return "\(aantal) meting\(aantal == 1 ? "" : "en") ontvangen"
    }
    ...
    var body: some View {
        NavigationStack {
            VStack(spacing: 40) {
                ...
                ZStack {
                    ...
                    Circle()
                        ...
                        .rotationEffect(.degrees(-90))
                        .frame(width: 110, height: 110)
                        .animation(.easeInOut(duration: 0.25), value: ringVoortgang)

                    VStack(spacing: 3) {
                        if ringVoortgang >= 0.999 {
                            Image(systemName: "checkmark")
                                .font(.system(size: 28, weight: .semibold))
                                .foregroundStyle(Color.appBlauw)
                        } else if bluetooth.ontvangenMetingAantal > 0 {
                            Text("\(bluetooth.ontvangenMetingAantal)")
                                .font(.system(size: 28, weight: .bold, design: .rounded))
                                .monospacedDigit()
                                .foregroundStyle(Color.appBlauw)
                            Text("meting\(bluetooth.ontvangenMetingAantal == 1 ? "" : "en")")
                                .font(.caption2)
                                .foregroundStyle(.secondary)
                        } else {
                ...
                VStack(spacing: 6) {
                    ...
                    Text(ontvangenTekst)
                        ...
                        .foregroundStyle(bluetooth.ontvangenMetingAantal > 0 ? Color.appBlauw : .secondary)
                        .monospacedDigit()
            ...
            .onChange(of: bluetooth.voltooideImporten) { _, _ in
